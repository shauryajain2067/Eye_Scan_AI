const firebaseConfig = {
  ...
  storageBucket: "silent-voice-ai.appspot.com", // ✅ FIXED: appspot.com
...
function signup(event) {
  ...
  const name = document.getElementById("name").value; // ✅ lowercase
  ...
  auth.createUserWithEmailAndPassword(email, password)
    ...
    .then(() => {
      ...
      window.location.href = "comms.html"; // ✅ moved here
    })
    .catch(error => {
  ...
  auth.signInWithPopup(provider)
    .then(result => {
      ...
      return db.ref("users/" + user.uid).once("value").then(snapshot => {
        if (!snapshot.exists()) {
          // First-time Google signup → save profile
          return db.ref("users/" + user.uid).set({
            email: user.email,
    ...
    .then(() => {
      ...
      window.location.href = "comms.html"; // ✅ redirect after success
    })
    .catch(error => {
